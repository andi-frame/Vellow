...
const BattlePage: React.FC = () => {
  ...
  const handleNextWinningCondition = () => setWinningCondition((prev) => (prev + 1) % winningConditions.length);
  const handlePrevWinningCondition = () =>
    setWinningCondition((prev) => (prev - 1 + winningConditions.length) % winningConditions.length);

  const handleNextDuration = () => setDurationIndex((prev) => (prev + 1) % durations.length);
  const handlePrevDuration = () => setDurationIndex((prev) => (prev - 1 + durations.length) % durations.length);

  const handleOpponentModeChange = () => setOpponentMode(opponentMode === "Invite" ? "Random" : "Invite");

  const handleLocationChange = () => setLocation(location === "Local" ? "Worldwide" : "Local");
  ...
  return (
    <div className="min-h-screen bg-[#1e1e1e] text-white">
      ...
      <div className="w-full max-w-md mx-auto p-4 flex flex-col items-center">
        <div className="mb-6 text-center w-full">
          ...
          <div className="flex justify-end">
            <h1 className="text-3xl font-bold animate-pulse">
              Z <span className="animate-pulse">z</span> <span className="animate-pulse">z</span>{" "}
          ...
          <div className="relative flex items-center justify-center">
            <div
              className="absolute w-[170px] h-[170px] bg-no-repeat bg-cover bg-center rounded-full"
              style={{ backgroundImage: "url('/noise.png')" }}></div>

            <Image src="/peluit.svg" alt="Battle" width={145} height={87} priority className="relative z-10" />
          </div>
        </div>
        ...
        <Image src="/gpvalue.svg" alt="gpvalue" width={14} height={14} priority />
        ...
        <div className="w-full flex flex-col items-center text-center mb-6 mt-6">
          <Image src="/winning.svg" alt="winning" width={14} height={14} priority />
          <h2 className="text-md font-semibold mb-2">Winning Condition</h2>
          <div className="flex items-center space-x-2">
            <button onClick={handlePrevWinningCondition}>{"<"}</button>
            <div className="border border-[#96A05B] px-4 py-2 rounded-md">{winningConditions[winningCondition]}</div>
          ...
          {winningConditions[winningCondition] === "Duration" ? (
            <div className="flex items-center space-x-2 mt-2">
              ...
              <div className="bg-[#C5D664] px-4 py-2 rounded-md">{durations[durationIndex]}</div>
        ...
        <div className="w-full flex flex-col items-center text-center mb-6 mt-3">
          <Image src="/opponent.svg" alt="opponent" width={14} height={14} priority />
          <h2 className="text-md font-semibold mb-2">Opponent</h2>
          <div className="flex items-center space-x-2">
            <button onClick={handleOpponentModeChange}>{"<"}</button>
            <div className="border border-[#96A05B] px-4 py-2 rounded-md">{opponentMode}</div>
            <button onClick={handleOpponentModeChange}>{">"}</button>
          </div>
        </div>
        ...
        {/* Location Selection */}
        <div className="w-full flex flex-col items-center text-center mb-6 mt-3">
          <Image src="/location.svg" alt="location" width={14} height={14} priority />
          <h2 className="text-md font-semibold mb-2">Location</h2>
          <div className="flex items-center space-x-2">
            <button onClick={handleLocationChange}>{"<"}</button>
            <div className="border border-[#96A05B] px-4 py-2 rounded-md">{location}</div>
            <button onClick={handleLocationChange}>{">"}</button>
          </div>
        </div>
        ...
        <Button
          className={`w-full max-w-[258px] h-[49px] py-4 mt-4 rounded-xl font-semibold transition mb-18 ${
            code.trim() !== "" && selectedType ? "bg-[#A467EE] hover:bg-purple-950" : "bg-gray-600 cursor-not-allowed"
          }`}
