...
import { loginWithProvider, fetchUser } from "@/lib/api";
...
const LoginStrava = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen text-white gap-10 w-sm">
      ...
      <div className="w-5/6 bg-[#A467EE] ring-1 ring-[#C69EF6] rounded-3xl flex flex-col justify-center items-center text-center p-5">
        ...
        <p className="text-sm font-light">
          To continue please authorize <span className="font-semibold">Strava</span>
        </p>
        <div className="flex flex-col py-16 w-2/3">
          <button
            className="rounded-2xl bg-[#1E1E1E]/20 px-4 
          py-3"
            onClick={() => loginMutation.mutate("strava")}
            disabled={loading}>
            {loading ? "Authorizing..." : "Authorize"}
          </button>
          <p className="text-xs font-light pt-1">
            Authenticate with <span className="font-semibold">Strava</span>
          </p>
        </div>
      </div>
    </div>
